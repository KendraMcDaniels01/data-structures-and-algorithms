...
const count = (target, input) => {
  const flatArray = input.reduce((accumulator, currentValue) => accumulator.concat(currentValue), []);

  const count = flatArray.reduce((accumulator, value) => {
    if (value === target) {
...
const totalSum = (input) => {
  ...
  const sum = flatArray.reduce((accumulator, value) => accumulator + value, 0);
...
let findMaleAndFemale = (data) => {
  const maleAndFemaleCharacters = data
    .filter((character) => character.gender === 'male' || character.gender === 'female')
    .map((character) => character.name);

  return maleAndFemaleCharacters.join(' and ');
};
...
let findShortest = (data) => {
  return data.reduce((shortest, character) => {
    if (!shortest || parseInt(character.height) < parseInt(shortest.height)) {
      return character;
    }
    return shortest;
  }, null).name;
