...
const screenForNames = (arr) => {
  const namePattern = /^(Mr\.|Mrs\.|Ms\.|Dr\.) [A-Za-z ]+$/;
  return arr.filter((name) => namePattern.test(name));
};
...
let biggerThanLuke = (arr) => {
  ...
  const heavierCharacters = arr.filter((character) => parseInt(character.mass) > lukeMass);

  const namesArray = heavierCharacters.map((character) => character.name);
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  const secureUrlPattern = /^https:\/\/[^/\s]+$/;
  return secureUrlPattern.test(url);
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let j = 0; j < 3; j++) {
    if (helpCheck(board, 0, j, 1, j, 2, j)) {
      return true;
    }
  }
